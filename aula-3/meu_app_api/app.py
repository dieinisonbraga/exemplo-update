...
def atualiza_produto(form: ProdutoSchema):
    ...
    nome_produto = form.nome # nome do produto que será atualizado
    
    logger.debug(f"Atualizando produto de nome: '{form.nome}'")
    try:
        # criando conexão com a base
        session = Session()
        # buscando o produto na base de dados
        updated_produto = session.query(Produto).filter(Produto.nome == nome_produto).first()

        # se o produto não estiver cadastrado na base de dados
        if not updated_produto:
            error_msg = "Produto não cadastrado no banco de dados :/"
            return {"mesage": error_msg}, 404
        else:
            # se foi informado um novo valor referente a quantidade
            if form.quantidade:
                updated_produto.quantidade = form.quantidade
            
            # se foi informado um novo valor referente ao valor
            if form.valor:
                updated_produto.valor = form.valor 
            
            session.add(updated_produto) #atualizando o produto na base de dados

            # efetivando o camando de atualização do item na tabela
            session.commit()
            logger.debug(f"Atualizando produto de nome: '{form.nome}'")
            return apresenta_produto(updated_produto), 200
